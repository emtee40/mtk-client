...
class ReadFlashWindow(QDialog):
    ...
    def updateDumpState(self):
        ...
        for partition in self.dumpStatus["allPartitions"]:
            ...
            if (self.dumpStatus["allPartitions"][partition]['done'] and partition != self.dumpStatus["currentPartition"]):
        ...
        self.partProgressText.setText("Current partition: "+self.dumpStatus["currentPartition"]+" ("+str(round((self.dumpStatus["currentPartitionSizeDone"]/1024/1024)))+"Mb / "+str(round((self.dumpStatus["currentPartitionSize"]/1024/1024)))+" Mb)");
    ...
    def dumpPartitionAsync(self, toolkit, parameters):
        ...
        for partition in self.partitionCheckboxes:
            if self.partitionCheckboxes[partition]['box'].isChecked():
                self.dumpStatus["allPartitions"][partition] = {"size": self.partitionCheckboxes[partition]['size'], "done": False}
        for partition in self.partitionCheckboxes:
            if self.partitionCheckboxes[partition]['box'].isChecked():
                variables = mock.Mock()
    ...
    def __init__(self, parent, mtkClass,da_handler,sendToLog):  #def __init__(self, *args, **kwargs):
        ...
        if guid_gpt is None:
            ...
            partitionListWidget = QWidget();
            partitionListWidgetVBox = QVBoxLayout(partitionListWidget)
            ...
            for partition in guid_gpt.partentries:
                ...
                self.partitionCheckboxes[partition.name]['box'].setText(partition.name+" ("+str(round(((partition.sectors * guid_gpt.sectorsize) / 1024 / 1024), 1)) + " Mb)")
        ...
        self.partProgressText.setGeometry(10, 365, 480, 40);
        ...
        self.startBtn.setGeometry((400 - 150 - 10), (500 - 30 - 10), 150, 30);
        self.startBtn.show();
